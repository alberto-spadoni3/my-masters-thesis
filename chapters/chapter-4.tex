...
La seguente analisi descrive l'architettura del nuovo database, mettendo in evidenza le principali differenze rispetto al sistema legacy. Le entità saranno presentate in gruppi tematici, organizzati in base alla loro funzione, fornendo una visione strutturata delle relazioni e delle caratteristiche di ciascuna componente del modello dati. In figura \Cref{fig:database-schema} è riportato lo schema concettuale del database, che rappresenta la struttura delle entità e delle relazioni principali.
...
Sebbene sia teoricamente possibile effettuare un'analisi per identificare i profili che potrebbero essere trasformati in condivisi, questa operazione risulta particolarmente complessa. Nel database legacy, infatti, non esiste un'informazione chiara e diretta che indichi se un profilo fosse utilizzato da più clienti. Questo rende difficile stabilire automaticamente quali profili potrebbero essere condivisi senza introdurre ambiguità o errori nella migrazione.

Per garantire una transizione coerente, si può quindi procedere migrando tutti i profili esistenti e impostandoli inizialmente come non condivisibili, riproducendo fedelmente la configurazione che gli utenti avevano nel vecchio sistema. Una volta completata la migrazione, saranno gli stessi amministratori ad avere la possibilità di convertire i profili in condivisi e riutilizzarli per più sottoclienti, sfruttando le nuove funzionalità del sistema.

\subsubsection{Migrazione delle categorie di blocco e delle eccezioni}
Per quanto riguarda l’applicazione delle policy di filtraggio, il meccanismo alla base è rimasto pressoché invariato tra il sistema legacy e il nuovo. Di conseguenza, la migrazione di questa componente consisterà principalmente nel trasferimento dei dati esistenti dalle tabelle del vecchio database alle corrispondenti del nuovo modello, assicurando che le associazioni tra profili e categorie di blocco siano mantenute intatte.

Una differenza sostanziale riguarda però la gestione delle eccezioni alle policy di filtraggio. Nel sistema legacy, le eccezioni erano archiviate in una semplice tabella contenente una lista di nomi di dominio, con l’indicazione se tali domini dovessero essere bloccati o consentiti. Ogni eccezione era direttamente associata a un codice profilo, rendendo la configurazione statica e non riutilizzabile tra profili differenti.

Nel nuovo sistema, invece, è stata introdotta la possibilità di creare gruppi di eccezioni riutilizzabili, separandoli dalla logica specifica dei profili. Di conseguenza, la migrazione dovrà prevedere un processo di trasformazione delle eccezioni esistenti, in modo da adattarle alla nuova struttura e garantire la coerenza tra i due ambienti. Una possibile soluzione consiste nel creare un gruppo di eccezioni per ogni profilo esistente, per poi andarlo a popolare con le relative eccezioni presenti nel vecchio database. Questo approccio permette di mantenere la stessa configurazione di eccezioni, garantendo al contempo la flessibilità e la riutilizzabilità offerte dal nuovo sistema.

\subsubsection{Migrazione delle reti}
La migrazione delle reti rappresenta un’altra sfida significativa, in quanto il nuovo sistema introduce una diversa modalità di rappresentazione degli indirizzi IP. Nel sistema legacy, una rete poteva essere identificata tramite un range di indirizzi, ma tali indirizzi venivano salvati nel database in maniera espansa. Ciò significava che un range contenente mille indirizzi IP veniva memorizzato come mille righe distinte all’interno della relativa tabella. Questo approccio comportava un’elevata ridondanza di dati e un notevole impatto in termini di spazio di archiviazione e operazioni di gestione.

Nel nuovo sistema, invece, la rappresentazione di una rete è stata ottimizzata mediante l’uso della notazione CIDR. Piuttosto che memorizzare ogni singolo indirizzo IP come un’entry separata, il range viene ora rappresentato come una stringa compatta, che viene poi elaborata a livello applicativo per determinare gli indirizzi contenuti al suo interno.

Di conseguenza, la migrazione dovrà prevedere una fase di consolidamento degli indirizzi IP memorizzati nel sistema legacy. Sarà necessario raccogliere tutti gli indirizzi appartenenti allo stesso range e convertirli nella relativa rappresentazione CIDR, che verrà poi inserita nel nuovo database. Questo processo ridurrà significativamente la quantità di dati memorizzati e migliorerà le operazioni di gestione e ricerca delle reti nel nuovo sistema.
